...
marketvalue <-
    ...
    ggplot +
        ...
        geom_col(alpha = alpha) +
        scale_y_continuous(breaks = 4e6 * 0:20, 
                           labels = scales::dollar_format(scale = 1 / 1e6, suffix = "M")) +
        labs(x = "Year",
             y = "Total market value (in $)",
...
turnover <-
    ...
    ggplot +
        ...
        geom_col(alpha = alpha) +
        scale_y_continuous(breaks = 0.02 * 0:10,
                           labels = scales::percent_format()) +
        labs(x = "Year",
             y = "Home turnover rate (in %)",
...
# overview <- (homessold + saleprice + marketvalue) / (timeonmarket + averageinventorysize) / (homeslisted + averageinventorytime + turnover)
